...
public class OverUnderBox extends Box {
   ...
   public String toString() {
	   return "OverUnderBox{base=" + base + ", del=" + del + ", script=" + script + "}";
   }
   ...
   /**
    * XML:
    *   <OverUnderBox kern=float over=bool>
    *     <Box> ... 基类的 ... </Box>
    *     <base> ...
    *     <del> ...
    *     <script> ...
   ...
   public void toXml(SimpleXmlWriter sxw, Object hint) {
	   sxw.appendRaw("<OverUnderBox")
	   	  .blank().attribute("kern", kern)
	   	  .blank().attribute("over", over);
	   super.addAttr(sxw);
	   sxw.appendRaw(">").ln();
	   ...
	   if (del != null) {
		   sxw.beginElement("del").ln();
		   del.toXml(sxw, hint);
		   sxw.endElement("del").ln();
	   }	   
	   if (script != null) {
		   sxw.beginElement("script").ln();
		   script.toXml(sxw, hint);
		   sxw.endElement("script").ln();
	   }
	   sxw.endElement("OverUnderBox").ln();
   }
}
