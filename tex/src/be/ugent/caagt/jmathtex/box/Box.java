...
/**
 * 公式的抽象图形表示, 可用于绘制. 所有的字符, 字体尺寸, 位置都是固定的. 只有特殊的 glue 盒子能伸展或压缩.
 * 
...
public abstract class Box {
   ...
   /**
    * 将 Box 的属性添加到 sxw 里面(派生类提供 element).
    * @param sxw
    */
   protected void addAttr(SimpleXmlWriter sxw) {
	   sxw.blank().attribute("width", width)
	      .blank().attribute("height", height)
	      .blank().attribute("depth", depth)
	      .blank().attribute("shift", shift);
	   
	   if (foreground != null)
		   sxw.blank().attribute("foreground", colorToString(foreground));
	   if (background != null)
		   sxw.blank().attribute("background", colorToString(background));
   }
    ...
    * @param sxw
    */
   protected void addChildren(SimpleXmlWriter sxw) {
	   if (children != null && children.size() > 0) {
		   for (int i = 0; i < children.size(); ++i) {
			   sxw.beginElement("children").ln();
			   children.get(i).toXml(sxw, this);
			   sxw.endElement("children").ln();
		   }
	   }
   }
