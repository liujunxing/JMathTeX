...
/**
 * 表示 `TeXFont' 的接口, 其责任是提供所有需要的字体和字体信息.
...
public interface TeXFont {
   ...
   // point-to-pixel conversion; 点到像素转换单位. (如果我们不是输出到屏幕, 该怎么办呢??)
   ...
   /**
    * 得到一个 Char 对象指定所给字符在所给字体中的尺寸信息...依赖与所给样式.
    ...
    * 		the given name
    */
   public Char getChar3(char c, String textStyle, int style) throws TextStyleMappingNotFoundException;
   ...
   /**
    * 得到指定符号(由名字 name 指定)的 Char-对象, 包含其尺寸信息(依赖于 style 的尺寸)
    * Get a Char-object for the given symbol with metric information depending on
    * "style".
    * 
    * @param symbolName the symbol name (符号名)
    * @param style the style in which the atom should be drawn (显示样式)
   ...
   /**
    * 以缺省 文本样式(text style), 和根据 显示样式(display style) 得到的尺寸信息, 
    *   获取(创建)一个 Char-对象.
    * Get a Char-object specifying the given character in the default text style with
    * metric information depending on the given "style".
    * 
    * @param c alphanumeric character; 只能是大写字母,小写字母,或数字.
    * @param style the style in which the atom should be drawn
    *   显示样式, 以决定字尺寸.
   ...
   /**
    * 为指定的符号(字符)获得一个扩展对象, 其包含4个可能的部分, 用于构建任意大小的字符变体.
    *   此函数只能对那些 isExtensionChar() 字符返回 true 的字符调用.
    *   推测: open/close 符号是 extension 的.
   ...
   /**
    * 以文本方式输出整个 TeXFont 的内容, 用于测试,调试,学习.
    */
   public void dump();
}
